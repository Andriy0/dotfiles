...
#
    ...
    # Lock the screen
    bindsym --to-code $mod+x exec swaylock -f -c 000000

    # Screenshot
    bindsym --to-code $mod+s exec grim -g "$(slurp)" - | wl-copy
...
for_window [title="Поділіться своєю новою зустріччю — Nightly"] floating enable
